...
#include <thread>
#include <variant>
static constexpr const char* taskService =
    "xyz.openbmc_project.RedfishPreserve";
static constexpr const char* taskInterface =
    "xyz.openbmc_project.RFPreserve.RedfishPreserve";
static constexpr const char* taskObject =
    "/xyz/openbmc_project/RedfishPreserve/taskHandle";
static constexpr const char* taskCreateInterface =
    "xyz.openbmc_project.RFPreserve.RFPmethod";
...
{
    PropertyValue value{};
    constexpr const char* dbuspropertyInterface =
    ...
    try
    {
        ...
        method.append("xyz.openbmc_project.RFPreserve.RedfishPreserve",
                      "TaskState");
        auto reply = bus.call(method);
        reply.read(value);
    }
...
inline void setServiceProperty(std::string& objPath, std::string property,
                               std::string value)
{
    sdbusplus::asio::setProperty(
        *crow::connections::systemBus, taskService, objPath, taskInterface,
        property, value, [&](const boost::system::error_code& ec) {
            if (ec)
            {
...
{
    std::string taskObjPath =
        "/xyz/openbmc_project/Task/task_" + std::to_string(index);
    setServiceProperty(taskObjPath, "TaskMessage", message);
}
...
inline void setTaskState(std::string state, size_t index)
{
    std::string taskObjPath =
        "/xyz/openbmc_project/Task/task_" + std::to_string(index);
    setServiceProperty(taskObjPath, "TaskState",
                       "xyz.openbmc_project.RFPreserve.RedfishPreserve.State." +
                           state);

    if (state == "Completed" || state == "Exception" || state == "Killed" ||
        state == "Cancelled")
    {
        BMCWEB_LOG_DEBUG("Setting End Time for Task Index : {} \r\n", index);
        setServiceProperty(taskObjPath, "EndTime", "" + getCurrentDateTime());
