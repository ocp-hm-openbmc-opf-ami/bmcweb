...
class Trie
{
  ...
  public:
    ...
    void add(std::string_view urlIn, unsigned ruleIndex)
    {
        ...
        Node& node = nodes[idx];
#if (!BMCWEB_AMI_REP_MACRO) && (!BMCWEB_AMI_NIC_MACRO) && (!BMCWEB_SPDM_URIS_MACRO) && (!BMCWEB_SBMR_EXT_MACRO) && (!BMCWEB_GPGPU_URIS_MACRO) && (!BMCWEB_DOT_URIS_MACRO) && (!BMCWEB_CPER_URIS_MACRO) && (!BMCWEB_NVIDIA_RESET_URIS_MACRO) && (!BMCWEB_NVIDIA_EROT_DUMP_MACRO) && (!BMCWEB_AMI_RM_MACRO) && (!BMCWEB_AMI_PSM_MACRO) && (!BMCWEB_AMI_THERMALEQUIPMENT_MACRO)
